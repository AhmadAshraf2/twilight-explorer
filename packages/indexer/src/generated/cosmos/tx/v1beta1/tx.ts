...
function createBaseTxRaw(): TxRaw {
  return { bodyBytes: Buffer.alloc(0), authInfoBytes: Buffer.alloc(0), signatures: [] };
}
...
export const TxRaw = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          message.bodyBytes = reader.bytes() as Buffer;
          continue;
        case 2:
          if (tag !== 18) {
            break;
          ...
          message.authInfoBytes = reader.bytes() as Buffer;
          continue;
        case 3:
          if (tag !== 26) {
            break;
  ...
  fromJSON(object: any): TxRaw {
    return {
      bodyBytes: isSet(object.bodyBytes) ? Buffer.from(bytesFromBase64(object.bodyBytes)) : Buffer.alloc(0),
      authInfoBytes: isSet(object.authInfoBytes) ? Buffer.from(bytesFromBase64(object.authInfoBytes)) : Buffer.alloc(0),
      signatures: globalThis.Array.isArray(object?.signatures)
        ? object.signatures.map((e: any) => Buffer.from(bytesFromBase64(e)))
        : [],
    };
  ...
  fromPartial<I extends Exact<DeepPartial<TxRaw>, I>>(object: I): TxRaw {
    const message = createBaseTxRaw();
    message.bodyBytes = object.bodyBytes ?? Buffer.alloc(0);
    message.authInfoBytes = object.authInfoBytes ?? Buffer.alloc(0);
    message.signatures = object.signatures?.map((e) => e) || [];
    return message;
  },
};
...
function createBaseSignDoc(): SignDoc {
  return { bodyBytes: Buffer.alloc(0), authInfoBytes: Buffer.alloc(0), chainId: "", accountNumber: "0" };
}
