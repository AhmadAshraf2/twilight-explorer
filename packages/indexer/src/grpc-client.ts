  ...
  return {
    ...
    block: {
      header: {
        ...
        time: header?.time ? new Date(header.time.seconds * 1000 + header.time.nanos / 1e6).toISOString() : new Date().toISOString(),
        last_block_id: {
      ...
      last_commit: {
        ...
        signatures: (block?.lastCommit?.signatures || []).map((sig: any) => ({
          ...
          timestamp: sig.timestamp
            ? new Date(sig.timestamp.seconds * 1000 + sig.timestamp.nanos / 1e6).toISOString()
            : '',
  ...
  async getTxsByHeight(height: number): Promise<{ tx_responses: TxResponse[]; pagination: Pagination }> {
    const resp = await unaryCall<any, any>(
      ...
      this.txService.getTxsEvent,
      {
        ...
        pagination: { limit: '100', countTotal: true },
