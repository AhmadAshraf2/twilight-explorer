...
export default function AccountDetailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* zkOS Operations */}
      {data?.zkosOperations && data.zkosOperations.length > 0 && (
        <div className="card">
          <h2 className="card-header flex items-center gap-2">
              ...
              <tbody>
                {data.zkosOperations.map((op: any) => (
                  <tr key={op.id}>
                    <td>
                      <span
                        className={`badge ${op.mintOrBurn ? 'badge-success' : 'badge-error'}`}
                      >
                        {op.mintOrBurn ? 'Mint' : 'Burn'}
                      </span>
                    </td>
                    <td className="font-medium">
                      {formatSatoshis(op.btcValue)}
                    </td>
                    <td>
                      <Link
                        href={`/blocks/${op.blockHeight}`}
                        className="text-primary-light hover:text-primary"
                      >
      ...
      {/* Fragment Signer Activity */}
      {data?.fragmentSigners && data.fragmentSigners.length > 0 && (
        <div className="card">
          <h2 className="card-header flex items-center gap-2">
                  ...
                  <tr key={signer.id}>
                    ...
                    <td>
                      <span className={`badge ${
                        signer.status === 'accepted' ? 'badge-success' :
                        signer.status === 'rejected' ? 'badge-error' :
                        'badge-warning'
                      }`}>
                        {signer.status}
                      </span>
                    ...
                    <td>
                      <Link
                        href={`/blocks/${signer.blockHeight}`}
                        className="text-primary-light hover:text-primary"
                      >
      ...
      {txData?.data && txData.data.length > 0 && (
        <div className="card">
          <h2 className="card-header flex items-center gap-2">
            ...
            Transactions ({txData.pagination.total})
          </h2>
          <div className="table-container">
            <table>
              <thead>
