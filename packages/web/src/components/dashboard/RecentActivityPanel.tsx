  ...
  return (
    <section className="card card-hover p-[22px]">
      ...
      <div className="mt-4 bg-black/30 border border-card-border rounded-[7px] overflow-hidden">
        {/* Header */}
        <div className="h-9 bg-background-tertiary/40 grid grid-cols-[1fr_1.5fr_1fr_1fr] items-center px-[10.5px]">
          ...
          {!transactions?.length ? (
            ...
            transactions.map((tx, idx) => (
              <div
                key={tx.hash}
                className={clsx(
                  'h-[38.5px] grid grid-cols-[1fr_1.5fr_1fr_1fr] items-center px-[10.5px]',
              ...
              >
                ...
                <Link
                  href={`/txs/${tx.hash}`}
                  className="font-mono text-[12.3px] leading-[18px] text-primary-light hover:text-primary transition-colors inline-flex items-center gap-2"
                >
